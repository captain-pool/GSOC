...
class SingleDeviceStrategy(object):
  ...
  def reduce(self, reduction_type, distributed_data, axis):
...
def train_and_export(**kwargs):
  if kwargs["tpu"]:
    ...
    assert kwargs["logdir"].startswith("gs://")
    assert kwargs["modeldir"].startswith("gs://")
    if kwargs["datadir"]:
      assert kwargs["datadir"].startswith("gs://")

  os.environ["TFHUB_CACHE_DIR"] = os.path.join(kwargs["modeldir"], "tfhub_cache")
...
if __name__ == "__main__":
  ...
  parser.add_argument("dataset", default=None, help="Name of the Dataset to use")
  parser.add_argument("datadir", default=None, help="Directory to store the downloaded Dataset")
  parser.add_argument("modeldir", default=None, help="Directory to store the SavedModel to")
  parser.add_argument("logdir",default= None, help="Directory to store the Tensorboard logs")
  parser.add_argument("tpu",  default=None, help="name or GRPC address of the TPU")
  parser.add_argument("num_steps", default=1000, help="Number of Steps to train the model for")
  parser.add_argument("--verbose", "-v", default=0, action="count", help="increase verbosity. multiple tags to increase more")
